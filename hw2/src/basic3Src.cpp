...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ImGui::ColorEdit3("clear color", (float*)&pick_color); // Edit 3 floats representing a color
		for (int i = 0; i < 3; i++)
		{
			vertices[i * 6 + 3] = pick_color.x;
			vertices[i * 6 + 4] = pick_color.y;
			vertices[i * 6 + 5] = pick_color.z;
		}
		glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
		ImGui::End();
		}
