...
void renderLoop(GLFWwindow* window)
{
	...
	*/
	...
	bool ortho = false;
	float orthoLeft = -5.f;
	float orthoRight = 1.5f;
	float orthoTop = -5.f;
	float orthoDown = 1.5f;
	...
	while (!glfwWindowShouldClose(window))
	{
...

		{
			...
			ImGui::Checkbox("origin", &origin);
			ImGui::Checkbox("moveCamera", &moveCamera);
			ImGui::Checkbox("bonus2", &bonus2);
			if (ortho)
			{
				ImGui::SliderFloat("left", &orthoLeft, -16.0f, 0.0f);
				ImGui::SliderFloat("rihgt", &orthoRight, 0.0f, 16.0f);
				ImGui::SliderFloat("top", &orthoTop, 0.0f, 16.0f);
				ImGui::SliderFloat("down", &orthoDown, -16.0f, 0.0f);
				ImGui::SliderFloat("near", &orthoNear, 0.1f, 50.0f);
				ImGui::SliderFloat("far", &orthoFar, 50.0f, 100.0f);
			}
			else
			{
				ImGui::SliderFloat("field of view", &perView, 0.0f, 90.0f);
				ImGui::SliderFloat("radio", &radio, 0.1f, 10.0f);
				ImGui::SliderFloat("near", &perNear, 0.1f, 50.0f);
				ImGui::SliderFloat("far", &perFar, 50.0f, 100.0f);
			}
			ImGui::End();
		}
		//深度测试
		...
		*/
		...
		if (ortho)
		{
			projection = glm::ortho(orthoLeft, orthoRight, orthoDown, orthoTop, orthoNear, orthoFar);
		}
		else
		{
		...
		if (moveCamera)
		{
			...
			model2 = glm::rotate(model2, glm::radians(-35.0f), glm::vec3(1.0f, 0.0f, 1.0f));
			model2 = glm::translate(model2, glm::vec3(1.5f, 3.5f, 1.5f));
			glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model2));
			glUniformMatrix4fv(viewLoc, 1, GL_FALSE, &view[0][0]);
